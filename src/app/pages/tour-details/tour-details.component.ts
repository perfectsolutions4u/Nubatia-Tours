...
export class TourDetailsComponent implements OnInit {
      ...
      this._DataService.getTourBySlug(this.slug).subscribe((res) => {
        if (res && res.data) {
          ...
          if (this.tour && this.tour.id) {
            ...
            // set background image from tour featured_image if available else use default image
            this.backgroundImage = this.tour.featured_image || '../../../assets/image/banner.webp';
  ...
  getReviews(id: number): void {
    this._DataService.getreviewByTourId(id).subscribe({
      next: (res) => {
        // console.log(res);
        if (res && res.data) {
          this.reviews = Array.isArray(res.data.data) ? res.data.data : [];
        } else {
          this.reviews = [];
        }
        this._cdr.markForCheck();
  ...
  galleryZoomLevel = 1;
  private readonly galleryZoomStep = 0.25;
  private readonly galleryZoomMin = 0.5;
  private readonly galleryZoomMax = 3;
  ...
  openGalleryLightbox(): void {
    if (this.tour?.gallery?.length) {
      this.galleryLightboxOpen = true;
      this.galleryCurrentIndex = 0;
      this.galleryZoomLevel = 1;
  ...
  galleryPrev(): void {
    if (!this.tour?.gallery?.length) return;
    this.galleryCurrentIndex =
      (this.galleryCurrentIndex - 1 + this.tour.gallery.length) % this.tour.gallery.length;
    this.galleryZoomLevel = 1;
  }

  galleryNext(): void {
    if (!this.tour?.gallery?.length) return;
    this.galleryCurrentIndex = (this.galleryCurrentIndex + 1) % this.tour.gallery.length;
    this.galleryZoomLevel = 1;
  }

  galleryZoomIn(): void {
    this.galleryZoomLevel = Math.min(this.galleryZoomMax, this.galleryZoomLevel + this.galleryZoomStep);
  }

  galleryZoomOut(): void {
    this.galleryZoomLevel = Math.max(this.galleryZoomMin, this.galleryZoomLevel - this.galleryZoomStep);
  }
